...
import { DEFAULT_DEALERSHIP, DEFAULT_NORMALIZATION_RULES, DEFAULT_USERS } from '../../config/defaults.js';
import { hashPassword } from '../../lib/auth.js';
import { prisma } from '../../lib/prisma.js';

export async function ensureSystemSeed(): Promise<Dealership> {
  ...
  const sourceConfigs: Array<{ source: SourceName; mode: InventorySourceMode; priority: number }> = [
...
export async function withDealershipTransaction<T>(
  dealershipId: string,
  run: (tx: Prisma.TransactionClient, dealershipId: string) => Promise<T>,
): Promise<T> {
  return prisma.$transaction((tx) => run(tx, dealershipId));
}
